...
for viewIdx = 1:4
    ...
    [fitResultX, gofX] = createFitx(Positions(:, 1)*0.4785, Positions(:, 2)*0.4785, xOffset*0.4785);
    [fitResultY, gofY] = createFitx(Positions(:, 1)*0.4785, Positions(:, 2)*0.4785, yOffset*0.4785);
...
figure;
t = tiledlayout(11, 11, 'TileSpacing', 'compact', 'Padding', 'compact');
flag = 0;
% Set min and max values
min_val = -8;  % Color-map minimum
max_val = 8;   % Color-map maximum
wavefront_all = nan(11*200+10*100,11*200+10*100);
for x_query = 1:101:1100
    for y_query = 1:101:1100
        flag = flag+1;
        % Evaluate the fit for each view
        for viewIdx = 1:4
            % Evaluate fitResult
        ...
        % Fit wavefront derivatives at this location
        [fitResult_fx, gofX] = createFit_f(fxyPosition(:,1),fxyPosition(:,2),fittedPosition(:,1)-fitted_center_pos(:,1));
        [fitResult_fy, gofY] = createFit_f(fxyPosition(:,1),fxyPosition(:,2),fittedPosition(:,2)-fitted_center_pos(:,2));
        ...
        % Define the grid
        [X, Y] = meshgrid(linspace(-0.825, 0.825, 200)); % Grid over [-1, 1], NA=0.66
        R = sqrt(X.^2 + Y.^2); % Radius
        theta = atan2(Y, X);  % Polar angle
        ...
        % Mask radii greater than 1 as NaN
        Wavefront(R > 0.825) = NaN;

        wavefront_all((((x_query-1)/101)*300+1):(((x_query-1)/101)*300)+200,(((y_query-1)/101)*300+1):(((y_query-1)/101)*300)+200) = Wavefront;
        ...
        surf(X, Y, Wavefront, 'EdgeColor', 'none');
        colormap(jet);
        % Set the color range of the current axes
        clim([min_val max_val]); % Colorbar min and max
        %colorbar;
        %title('Wavefront Reconstruction');
        %xlabel('X');
        %ylabel('Y');
...
figure;
surf(wavefront_all, 'EdgeColor', 'none');
colormap(jet);
% Set the color range of the current axes
clim([min_val max_val]); % Colorbar min and max
%colorbar;
